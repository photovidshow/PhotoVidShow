...
//static const char RCSID[]="$Id: //depot/dvdauthor/src/subfont.c#10 $";
...
#define WARNING(msg, args...)      fprintf(stderr,"" msg "\n", ## args)
...
/*	fprintf(stderr,"INFO: get_path('%s') -> '%s'\n",filename,buff); */
	return buff;
}
...
//    fprintf(stderr, "select unicode charmap: %d\n", error);
	...
	if (character==0)
	    ...
	    if (glyph_index==0) {
		WARNING("Glyph for char 0x%02x|U+%04X|%c not found.", (unsigned int)code,(unsigned int)character,
...
//	fprintf(stderr,"ERR: Iconv doesn't know %s encoding. Use the source!\n", charmap);
    ...
    if (cd==(iconv_t)-1) {
//	fprintf(stderr,"ERR: Unsupported encoding `%s', use iconv --list to list character sets known on your system.\n", encoding);
	return -1;
    }
...
static int load_sub_face(char *name, FT_Face *face)
{
    ...
    if (err) {
	err = FT_New_Face(library, get_path(sub_font), 0, face);
	if (err) {
#if 0
            char *mp_sub_font;
...
#endif
	    if (err) {
	       // fprintf(stderr,"ERR: New_Face failed. Maybe the font path is wrong.\nPlease supply the text font file (%s).\n",sub_font );
		return -1;
	    }
	}
    }

...
//static const char RCSID[]="$Id: //depot/dvdauthor/src/dvdauthor.c#72 $";
...
int getframepts(struct vobgroup *va)
{
...
int getcellpts(struct vob *va,int cellid)
{
...
int findvobu(struct vob *va,int pts,int l,int h)
{
    // int l=0,h=va->numvobus-1;
...
int getptsspan(struct pgc *ch)
{
    int ptsspan=0,s,c,ci;

...
unsigned long htonl(unsigned long netlong)
{
	B2N_32(netlong);
	return netlong ;
...
char* g_srg_raw_main_image;
char* g_srg_raw_highlight_image;
char* g_srg_raw_select_image;


//static const char RCSID[]="$Id: //depot/dvdauthor/src/subgen.c#48 $";
...
static int sread(int h,void *b,int l)
{
    ...
    while(l>0) {
        int r=read(h,b,l);
        if( r==-1 ) {
         //   DebugOutbutMessage("WARN:  Read error %s\n",strerror(errno));
            return -1;
        }
        if( !r ) {
         //   DebugOutbutMessage("WARN:  Read %d, expected %d\n",tr+r,tr+l);
            return tr;
        }
        l-=r;
        b=((unsigned char *)b)+r;
...
static void swrite(int h,void *b,int l)
{
    ...
    while(l>0) {
        int r=write(h,b,l);
        if( r==-1 ) {
         //   DebugOutbutMessage("ERR:  Write error %s\n",strerror(errno));
            exit(1);
        }
        l-=r;
        b=((unsigned char *)b)+r;
...
//	    fprintf(stderr,"%d:%02d:%02d.%03d\r",
  //          (int)(s->spts/90/1000/60/60),
   ...
   //         (int)(s->spts/90)%1000);
        ...
        if(process_subtitle(s,g_srg_raw_main_image,g_srg_raw_highlight_image,g_srg_raw_select_image))
            return s;
        freestinfo(s);
        skip++;
    }
    ...
    while( newsti ) {
        ...
        if( (cursti->sd == -1) && newsti && ( (!svcd) || until_next_sub) )
        {
            ...
            else
            {
                if (debug > -1)
                {
                //    DebugOutbutMessage("ERR: Sub with too short or negative duration on line %d, skipping\n",\
                            spuindex - 1);
                }
                exit(1);
...
char PACKAGE_NAME[] ="rgjrkrekgrekgkrekgkrgkrekgkergkrkegkrekgkerkgerkgkerg";

extern int had_stream;
extern stinfo *st;
extern button *curbutton;

extern int srg_video_width;
extern int srg_video_height;


int subuxmain(char* my_xml_buffer, char* input_name, char* output_name, char* srg_raw_main_image, char* srg_raw_highlight_image, char* srg_raw_select_image, int the_mode, int width, int height, struct centre_button_buttons* the_button_pos)
{
    int fdi;
    unsigned int c, ch, a, frame;
    unsigned short int b,bb, vss;
    unsigned char psbuf[psbufs], ncnt;
    int optch;
	button * but;

	srg_video_width = width;
	srg_video_height = height;

	
	//DebugOutbutMessage("Doing sm2 in mode %d!!", the_mode);

//	fprintf(stderr,"Doing subixmain a !!\n");

	g_srg_raw_main_image = srg_raw_main_image;
	g_srg_raw_highlight_image = srg_raw_highlight_image;
	g_srg_raw_select_image = srg_raw_select_image;

	//fprintf(stderr,"Doing subixmain2!!\n");

    newsti=0;
    mode = the_mode; // default DVD 
    sub = malloc( SUB_BUFFER_MAX + SUB_BUFFER_HEADROOM );
    if(! sub)
    {
//	DebugOutbutMessage("ERR: Could not allocate space for sub, aborting.\n");
...
//fprintf(stderr, "malloc sub=%p\n", sub);
	...
	//DebugOutbutMessage("Doing sm3\n");

    image_init();

//	DebugOutbutMessage("Doing sm4\n");

//    fputs(PACKAGE_HEADER("spumux"),stderr);

	//fprintf(stderr,"done image_init!!\n");
...
*/
	...
	*/
	...
	fdi = open(input_name, O_RDONLY | O_BINARY);
	if (fdi < 0) {
		DebugOutbutMessage("error opening %s\n",input_name);

	    exit(-1);
	}

//	DebugOutbutMessage("Doing subixmain5\n");

	fdo = open(output_name, _O_CREAT | _O_RDWR | O_BINARY);
	if (fdo < 0) {
	    DebugOutbutMessage("error opening %s\n",output_name);
	...
	//DebugOutbutMessage("Doing subixmain6\n");

    win32_setmode(fdi,O_BINARY);
//	DebugOutbutMessage("Doing subixmain6.1\n");
    win32_setmode(fdo,O_BINARY);
	//DebugOutbutMessage("Doing subixmain6.2 %s \n",my_xml_buffer);
	if( spumux_parse(my_xml_buffer) )
	{
		DebugOutbutMessage("Doing ERROR return -1\n");
        return -1;
	}

	

//	DebugOutbutMessage("Doing sm.5\n");

    if(tofs>=0  &&  (debug > 0) )
	//printf("INFO: Subtitles offset by %fs\n", (double)tofs / 90000);
	...
	if (spus!=NULL)
	{
		stinfo* current_spu = *spus;

		int ii=0;
	//	DebugOutbutMessage("Found spus = %d\n", numspus);
		...
		if (numspus>0)
		{
			...
			//	DebugOutbutMessage("Found buttons = %d\n", current_spu->numbuttons);
				for (hh=0;hh <current_spu->numbuttons;hh++)
				{
					int centre_x =0;
					int centre_y =0;
					
			//		DebugOutbutMessage("Found BUTTON %d\n", hh);

					but = &current_spu->buttons[hh];

				//	DebugOutbutMessage("x0 = %d\n",but->r.x0);
				//	DebugOutbutMessage("y0 = %d\n",but->r.y0);
				//	DebugOutbutMessage("x1 = %d\n",but->r.x1);
				//	DebugOutbutMessage("y1 = %d\n",but->r.y1);
					centre_x= but->r.x1 - but->r.x0;
					centre_x/=2;
					centre_x+=but->r.x0;

					centre_y = but->r.y1 - but->r.y0;
					centre_y/=2;
					centre_y+=but->r.y0;

					the_button_pos->x0[the_button_pos->num_buttons] = centre_x;
					the_button_pos->y0[the_button_pos->num_buttons] = centre_y;
					the_button_pos->num_buttons++;

					// this should never happen as we only have possible 8
					if (the_button_pos->num_buttons>10)
					{
						break;
					}
				}
	...
	{
		//DebugOutbutMessage("spus were null");
	}
    ...
    while(1)
    {
		//DebugOutbutMessage("Doing subixmain7");
	...
	if(ch == 0x1ba) // packet start code 
        ...
        l_01ba:
            if(progr)
            {
             //   if (lps % 1024 * 1024 * 10 < secsize)
               //     printf("INFO: %lld bytes of data written\r", lps);
            }

            if(debug > 5) 
			{
			//	printf("INFO: pack_start_code\n");
			}
            ...
            if(gts != -1) {
                ...
                if (debug >- 1)
				{
              //      printf("WARN: Incorrect pack header\n");
				}
                gts=nextgts;
            }
            mkpackh(gts,muxrate,0);
	...
	else if( ch>=0x1bb && ch<=0x1ef )
        {
            ...
            if (debug > 0)
            {
        //        printf("WARN: Unknown header %.2x %.2x %.2x %.2x\n",\
         ...
         //   printf("INFO: Skipped %d bytes of garbage\n", b);
...
//    fprintf(stderr, "max_sub_size=%d\n", max_sub_size); 
    ...
    if (subno  !=  0xffff)
    {
	//fprintf(stderr, "INFO: %d subtitles added, %d subtitles skipped, stream: %d, offset: %.2f\n",\
      //          subno + 1, skip, substr, (double)tofs / 90000);
//	DebugOutbutMessage("INFO: %d subtitles added, %d subtitles skipped, stream: %d, offset: %.2f\n",\
  //              subno + 1, skip, substr, (double)tofs / 90000);
    }
    else
    {
//	fprintf(stderr,"WARN: no subtitles added\n");
//	DebugOutbutMessage("WARN: no subtitles added\n");
	}
//	DebugOutbutMessage("Doing sm7\n");

	
//	DebugOutbutMessage("Doing sm8\n");


	// ok lets output spus info
	...
	subno=secsize=mode=fdo=header_size=muxrate=0;
	substr=0;
	sector=0;
	newsti=0;
	gts = nextgts=0;
	...
	have_textsub=0;
	have_transparent=1;
	transparent_color=0x808080;
	header[0]=0;
	//DebugOutbutMessage("Doing sm END\n");

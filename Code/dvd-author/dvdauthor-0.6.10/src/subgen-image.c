...
//static const char RCSID[]="$Id: //depot/dvdauthor/src/subgen-image.c#20 $";
...
static void putpixel(pict *p,int x,palt *c)
{
	...
	// loop through pallete lookinf for matching colour
    for( i=0; i<p->numpal; i++ )
	{
	...
	// to many colours used  only 256 please
    if( p->numpal==256 ) 
	{
        p->img[x]=0;
        return;
...
static int read_magick(pict *s)
{
   ...
   // fprintf(stderr,"INFO: Picture had %d colors\n",s->numpal);
...
// ************************* read from memory
void srg_read_raw(stinfo *s, pict *pic)
{

  int x,y;

  palt our_t;
  our_t.b = our_t.g = our_t.r = our_t.t =0;

  createimage(pic,srg_video_width,srg_video_height);

  for( y=0; y<srg_video_height; y++ )
  {
	unsigned char *d=srg_raw_image+(y*srg_video_width*4);

    for( x=0; x<srg_video_width; x++ )
	{
      palt p;
	  p.b=*d++;
      p.g=*d++;
	  ...
	  // first pixel is transparent colour
	  if (pic->numpal == 0)
	  {
		    p.t=0;
			our_t = p;
	  }
	  // if we come accross that colour again make in transparent
	  else if (our_t.b == p.b && our_t.g == p.g && our_t.r == p.r)
	  {
		  p = our_t;
	  }

	  putpixel(pic,y*pic->width+x,&p);
	}
  }
...
static void brphelp(int a0,int a1,int b0,int b1,int d,int *angle,int *dist)
{
    ...
    *angle=m*180/3.14159265358979323846*atan(((double)d1)/((double)d));
    *dist=sqrt(d1*d1+d*d);
}
  ...
  //  FILE* fp;
    ...
    if( s->numbuttons ) {
        ...
        for( i=0; i<s->numbuttons; i++ ) {
            ...
            // SRG there seems to be some bug, where we need to add another pixel to width and height to the mask image (hence we have <= not <)
            for( y=b->r.y0; y<=b->r.y1; y++ )
                for( x=b->r.x0; x<=b->r.x1; x++ ) {

                    // SRG make sure inside image
                    if (y<h && x<w)
                    {
                        int dc=-1,p=y*w+x,j;
    ...
    */
    ...
    /* go through all final image pixels and set remaining 255 pixels */
    for( i=0; i<w*h; i++ )
        if( s->fimg[i]==255 ) {
            int j;
    ...
    */
    ...
    // determine minimal visual area (boundary box around all subtitles in image), and crop the subtitle accordingly, x0,y0,xd,yd represent the new cropped subbititle image
    // This is here i assume to reduce output size by a little bit
    x0=w;
    y0=-1;
    s->xd=0;
    s->yd=0;
...
int process_subtitle(stinfo *s, char* srg_raw_main_image, char * srg_raw_highlight_image, char *srg_raw_select_image)
{
    int w=0,h=0;
    int iline=0;

	srg_raw_image = srg_raw_main_image ;

    if( !s ) return 0;
	if( read_pic(s,&s->img) ) {
        if(debug > -1)
           // fprintf(stderr, "ERROR: Bad image,  skipping line %d\n", iline - 1);
	//	DebugOutbutMessage("ERROR!!!!: Bad image %s,  skipping line\n", s->img.fname);
        return 0;
    }
   	if( s->img.img && !w ) {
        w=s->img.width;
	...
	srg_raw_image = srg_raw_highlight_image ;
	...
	srg_raw_image = srg_raw_select_image ;

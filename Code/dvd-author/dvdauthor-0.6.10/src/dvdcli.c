...
void DebugOutbutMessage(char *fmt, ...)
{
	FILE* fp = NULL;
	if ( fmt )
	{
...
//static const char RCSID[]="$Id: //depot/dvdauthor/src/dvdcli.c#50 $";

#define RGB2YCrCb(R,G,B) ((((int)RGB2Y(R,G,B))<<16)|(((int)RGB2Cr(R,G,B))<<8)|(((int)RGB2Cb(R,G,B))))

extern int readdvdauthorxml(const char *my_xml_buffer,char *fb);
...
int getopt2(char a,char b, char c) {} 

extern void reset_static_variables();

int createdvdfiles(char* my_xml_buffer, char* output_name)
{
...
#define GETOPTFUNC(x,y,z) getopt2(x,y,z)
#endif
...
//    fputs(PACKAGE_HEADER("dvdauthor"),stderr);
	...
	readdvdauthorxml(my_xml_buffer,output_name);
   ...
   //}
    ...
    while(1) {
        ...
        case 'x':
            ...
            return readdvdauthorxml("onesource.xml",fbase);
        ...
        case 's':
            MAINDEF;
			FORCE_CRASH();
            parsesubpictureopts(vc,"A");
            break;

        case 'b':
            MAINDEFPGC;
			FORCE_CRASH();
            parsebutton(curpgc,"A");
            break;

        case 'i':
            MAINDEFPGC;
			FORCE_CRASH();
            parseinstructions(curpgc,"a");
            break;
        ...
        case 'e':
            ...
            parseentries(curpgc,"a");
            break;

		case 'P': { FORCE_CRASH();  }
		case 'p': { FORCE_CRASH(); 
            MAINDEFPGC;
            readpalette(curpgc,"xste-palette.dat");
			break;}
    ...
    if( !fbase ) {
        ...
        if( !fbase ) {
         //   fprintf(stderr,"ERR:  Must specify working directory\n");
			DebugOutbutMessage("Error getfbase\n");
            parser_err=1;
        }
    }
}
...
static void vmgm_start()
{
    if( hadtoc ) {
      //  fprintf(stderr,"ERR:  Can only define one VMGM\n");
		DebugOutbutMessage("error vmgm_start\n");
        parser_err=1;
        return;
    }
    mg=menugroup_new();
...
static void fpc_start()
{
    ...
    //    fprintf(stderr,"ERR:  Can only specify <fpc> under <vmgm>\n");
        parser_err=1;
		DebugOutbutMessage("error fpc_start\n");
        return;
    }
    if( fpc ) {
    //    fprintf(stderr,"ERR:  Already defined <fpc>\n");
        parser_err=1;
		DebugOutbutMessage("error fpc_start fpc\n");
        return;
    }
    fpc=pgc_new();
    parser_acceptbody=1;
...
static void titles_start()
{
    if( titles ) {
     //   fprintf(stderr,"ERR:  Titles already defined\n");
        parser_err=1;
		DebugOutbutMessage("error titles_start1\n");
    } else if( istoc ) {
    //    fprintf(stderr,"ERR:  Cannot have titles in a VMGM\n");
		parser_err = 1;
		DebugOutbutMessage("error titles_start2\n");
    } else {
        titles=pgcgroup_new(0);
        curgroup=titles;
        ismenuf=0;
...
static void video_start()
{
    if( setvideo ) {
     //   fprintf(stderr,"ERR:  Already defined video characteristics for this PGC group\n");
        parser_err=1;
		DebugOutbutMessage("error video_start\n");
...
static void subattr_start()
{
    ...
    if( setsubpicture>=32 ) {
       // fprintf(stderr,"ERR:  Attempting to define too many subpicture streams for this PGC group\n");
        parser_err=1;
		DebugOutbutMessage("error subattr_start\n");
    }
}
...
static int readdvdauthorxml(const char *my_xml_buffer,char *fb)
{
    fbase=fb;
    return readxml(my_xml_buffer,elems,attrs);
}

...
//static const char RCSID[]="$Id: //depot/dvdauthor/src/dvdvob.c#49 $";
...
int FindVobus(char *fbase,struct vobgroup *va,int ismenu)
{
    ...
    for( vnum=0; vnum<va->numvobs; vnum++ ) {
        int i,j;
        int hadfirstvobu=0,backoffs=0,lastscr=0;
       ...
       // fprintf(stderr,"\nSTAT: Processing %s...\n",s->fname);
        ...
        while(1) {
            ...
            if( buf[14]==0 && buf[15]==0 && buf[16]==1 && buf[17]==0xbe && !strcmp(buf+20,"abcdefghijklm4")) {
            ...
            if( buf[0] == 0 &&
                ...
                buf[21] & 128 ) {
                int pts0=readpts(buf+23),pts1,backpts1;
		...
		case 1: { FORCE_CRASH(); /*pclose(h);*/ break; }
        case 2: break;
        }
        if( s->numvobus ) {
            int i,finalaudiopts;
            ...
            for( i=0; i<s->numvobus; i++ ) {
                ...
                if( vi->hasvideo ) {
                    vi->videopts[0]=calcpts(va,vi->firstvideopts,-vi->firstIfield);
                    vi->videopts[1]=calcpts(va,vi->videopts[0],vi->numfields);
            ...
            for( i=0; i<s->numvobus; i++ ) {
                ...
                if( !vi->hasvideo ) {
                   ...
                   //     fprintf(stderr,"WARN: Cannot detect pts for VOBU if there is no audio or video\nWARN: Using SCR instead.\n");
                    ...
                    if( i ) {
                        j=firstaudiopts-s->vi[i-1].sectpts[0];
                        // ensure this is a multiple of a framerate, just to be nice
                        k=getframepts(va);
                        j+=k-1;
                        j-=j%k;
                        j+=s->vi[i-1].sectpts[0];
                        assert(j>=s->vi[i-1].sectpts[1]);
                        s->vi[i-1].sectpts[1]=j;
                    } else {
                    ...
                    if( i+1==s->numvobus ) { // if this is the end of the vob, use the final audio pts as the last pts
                        if( finalaudiopts>vi->sectpts[0] )
                            ...
                            j=vi->sectpts[0]+getframepts(va); // add one frame of a buffer, so we don't have a zero (or less) length vobu
...
static int findnearestvobu(struct vobgroup *pg,struct vob *va,int pts)
{
    ...
    if( i+1<va->numvobus && i>=0 &&
        abs(pts-va->vi[i+1].sectpts[0])<abs(pts-va->vi[i].sectpts[0]) )
        i++;
    return i;
}
...
static int getcellaudiopts(struct vobgroup *va,int vcid,int ach,int w)
{
...
static int getcellvideopts(struct vobgroup *va,int vcid,int w)
{
    struct vob *v=va->vobs[(vcid>>8)-1];
    int vi=0;
...
static int calcaudiodiff(struct vobgroup *va,int vcid,int ach,int w)
{
...
int calcaudiogap(struct vobgroup *va,int vcid0,int vcid1,int ach)
{
    ...
    if( (vcid1&255)==1 && va->vobs[(vcid0>>8)-1]->numcells==(vcid0&255) ) {
        ...
        if( hasaudio(va,vcid0,ach,1) && !hasaudio(va,vcid1,ach,0) ) {
           // fprintf(stderr,"WARN: Transition from non-audio to audio VOB; assumjing discontinuity.\n");
            return 1;
        }

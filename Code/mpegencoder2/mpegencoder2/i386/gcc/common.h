...
struct AVOption;
#ifdef HAVE_MMX
//extern const struct AVOption avoptions_common[8];
#else
...
static __inline int get_bits_trace(GetBitContext *s, int n, char *file, char *func, int line){
    int r= get_bits(s, n);
...
static __inline int get_vlc_trace(GetBitContext *s, VLC_TYPE (*table)[2], int bits, int max_depth, char *file, char *func, int line){
...
static __inline int get_xbits_trace(GetBitContext *s, int n, char *file, char *func, int line){
...
static __inline int clip(int a, int amin, int amax)
{
    if (a < amin)
        return amin;
    else if (a > amax)
...
static __inline int ff_sqrt(int a)
{
    int ret=0;
    int s;
    int ret_sq=0;
...
#ifdef ARCH_X86
static __inline long long rdtsc()
{
	long long l;
	asm volatile(	"rdtsc\n\t"
		: "=A" (l)

...
static __inline void mpeg4_encode_block(MpegEncContext * s, DCTELEM * block,
...
static __inline int ff_mpeg4_pred_dc(MpegEncContext * s, int n, int level, int *dir_ptr, int encoding);
...
static void aspect_to_info(MpegEncContext * s, AVRational aspect){
	...
	if(aspect.num==0) { aspect.den=1; aspect.num=1; } 
    for(i=1; i<6; i++){
...
static __inline int decide_ac_pred(MpegEncContext * s, DCTELEM block[6][64], int dir[6], uint8_t *st[6], int zigzag_last_index[6])
{
    int score= 0;
    int i, n;
...
static __inline void restore_ac_coeffs(MpegEncContext * s, DCTELEM block[6][64], int dir[6], uint8_t *st[6], int zigzag_last_index[6])
{
    int i, n;
...
static __inline int mpeg4_get_block_length(MpegEncContext * s, DCTELEM * block, int n, int intra_dc, 
                               uint8_t *scan_table)
{
    int i, last_non_zero;
    const RLTable *rl;
...
int h263_decode_picture_header(MpegEncContext *s)
{
    ...
    */
    ...
    if (format != 7 && format != 6) {
        ...
        s->width = width;
        s->height = height;
		s->avctx->sample_aspect_ratio.num=12;  //= (AVRational){12,11};

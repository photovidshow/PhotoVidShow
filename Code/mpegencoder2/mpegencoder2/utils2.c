...
int av_new_packet(AVPacket *pkt, int size)
{
    ...
    memset(((unsigned char*)data) + size, 0, FF_INPUT_BUFFER_PADDING_SIZE);
...
// covert raw video frame into avpackt
static int get_next_video_frame(AVPacket* pkt)
{
...
extern int srg_frames_total;
extern int srg_abort;
...
static int av_read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
	...
	//SRG
	//return get_next_video_frame(pkt);

	srg_frames_done++;
	if (srg_frames_done > srg_frames_total || srg_abort==1)
	{
		return -EAGAIN;
	}
...
static void av_estimate_timings(AVFormatContext *ic)
{
    ...
    if ( /*SRG HACKic->iformat == &mpegps_demux*/ 1==2) {

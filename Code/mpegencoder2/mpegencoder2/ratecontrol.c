...
int ff_rate_control_init(MpegEncContext *s)
{
	...
	// SRG HACK TO FIX SMALL CLICHE AT START
	// THE rc_initial_buffer_occupancy SEEMS TO BE 0 which is passed to buffer index and then used
	// IN ESTIMATION FOR QSCALE.  IT THEN SETS QSCALE HIGH as possible (v low quality) i assume to
	// fit in a buffer of 0 !!!.  the buffer index then goes to say something a lot lower than buffer size
	// but bigger than 0, so next estimation it will be slightly less low quality and so on (normaly for about
	// 2 seconds until things settle down)
	
	//IT SEEMS TO ME THAT THIS INITIAL VALUE SHOULD BE THE SAMEAS RC_BUFFER, 
	//(i.e. we want to estimate a scale for a frame which used up all the buffer)

	// SRG FUTHER NOTES,   i THINK AS LONG AS more than MIN BUFFER SIZE THEN OK (i.e not 0)
	// I"M STILL A BIT CONFUSED ABOUT THIS RC_BUFFER_SIZE WITCH WE HAVE UPPED IN mpegvideo.c
	// ??!!??!!??!!??!!??!!??  

	rcc->buffer_index = s->avctx->rc_buffer_size;

	//sprintf(temp,"!!!!!!!!!initial rc buffer set to %f",rcc->buffer_index);
//	av_log(NULL,NULL,temp);
...
#ifdef _DEBUG
            mpeg2_error("rc buffer underflow\n");
#endif
            rcc->buffer_index= 0;
        }

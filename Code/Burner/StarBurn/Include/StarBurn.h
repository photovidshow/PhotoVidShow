...
Revision/Development History:
	...
	2008-10-13 : * StarBurn_UDF2_VolumeAnchorGet(...) API call added.
			   :
			   : * UDF volume creation APIs extended to pass OS class and initial volume offset in logical blocks.	

	2008-10-15 : * Overwrapped file I/O and UDF import volume related functions added.

	2008-10-19 : * StarBurn_CdvdBurnerGrabber_GetLastAddress(...) API call added.

	2008-10-20 : * StarBurn_UDF2_ImpVolumeCreateEx(...) API call has been dropped. 
			   :
			   : * StarBurn_UDF2_ImpVolumeCreate(...) API call parameters list changed.

	2008-11-15 : * StarBurn_CdvdBurnerGrabber_TrackAtOnceFromFileAudioUnicodeEx(...) API call added.
...
typedef struct _STARBURN_FILE_OBJECT *PSTARBURN_FILE_OBJECT; 
...
// Remarks
//
// Check StarBurn_UDF2_ImpVolumeImport(...) API call

typedef 
LONG 
( __stdcall *PSTARBURN_UDF2_IMPORT_CALLBACK )( 
	LONG p__LONG__LBA,
	PVOID p__PVOID_Buffer,
	ULONG p__ULONG__BufferSizeInUCHARs,
	PVOID p__PVOID__Context 
	);
...
p__PCHAR__BootImageFileName - Pointer to boot image file name with that be saved on disc.

p__ULONG__BootImageFileAttributes - Boot image file attributes.
...
p__USHORT__LoadSegment - Physical address of the memory where will be written code from the first sector of boot image. 

p__BOOLEAN__IsHideNode - Indicates will the node be present in the root folder or not

p__BOOLEAN__IsBootInformationPatch - Is recalculate CRC of the boot image.
...
p__PVOID__UDF__Parent - Pointer to file tree node
...
p__PCHAR__DeviceName - Pointer to device symbolic link name we'll use. Make sure it DOES NOT include any slashes. So the name
should be either "CdRom0" or say "F:" but NOT "\\.\CdRom0" or "\\.\F:". 
...
This function gets last recorded address from CD/DVD/Blu-Ray/HD-DVD media currently inserted to CD/DVD/Blu-Ray/HD-DVD burner device object. Such an
address could be used for already recorded session import with the UDF mastering.
...
p__PULONG__LBA - Pointer to the variable to receive last recorded address.
...
Please see UDF2ImportBurn sample that will demonstrate how UDF file system image can be burn on the CD/DVD/Blu-Ray/HD-DVD media and how 
StarBurn_CdvdBurnerGrabber_GetLastAddress can be used to get last recorded address.
...
CHAR l__CHAR__ExceptionText[ 1024 ];
CDB_FAILURE_INFORMATION l__CDB_FAILURE_INFORMATION;
ULONG l__ULONG__LastRecordedLBA = 0;
...
StarBurn_Destroy, StarBurn_CdvdBurnerGrabber_Create, PCALLBACK, EXCEPTION_NUMBER, CDB_FAILURE_INFORMATION,
StarBurn_CdvdBurnerGrabber_TrackAtOnceFromTree, StarBurn_CdvdBurnerGrabber_GetLastTrack
...
p__PCDB_FAILURE_INFORMATION - Pointer to CDB failure information.

p__PWCHAR__SoundImagePathAndFileName - Pointer to sound path and file name in Unicode format.

p__BOOLEAN__IsXA - Is this CDROM XA or just ordinary CDROM/CDDA.

p__BOOLEAN__IsTestWrite - Is this test write.

p__BOOLEAN__IsNextSessionAllowed - Is next session allowed.
...
p__BOOLEAN__IsAudioPauseChange - Should we change audio pause length.
...
p__PDAO_DISC_LAYOUT - Pointer to DAO disc layout.
...
p__BOOLEAN__IsSubChannelRepairRequired - Is subchannel repair required.
...
ISO9660FileTree - Pointer to pointer to the object that toolkit will set to the ISO 9660 File Tree object it will allocate.
...
If success then return ERROR_SUCCES, otherwise error code.
...
Directory - Pointer to pointer to directory object.

Name - Pointer to zero-terminated directory name.
...
UnicodeName - Pointer to zero-terminated directory name (UNICODE format).
...
File - Pointer to pointer to file object.

Name - Pointer to zero-terminated name.

PathName - Pointer to zero-terminated path and name.
...
Name - Pointer to zero-terminated name (UNICODE format).

PathName - Pointer to zero-terminated path and name (UNICODE format).
...
MemoryRegion - Pointer to memory region.

MemoryRegionSizeInUCHARs - Memory region size in unsigned chars.
...
Volume - Pointer to pointer to volume object.

Root - Pointer to root directory.

Name - Pointer to zero-terminated string with volume name.

IsUnicode - Is UNICODE (should we convert single character nodes to UNICODE).

IsGlobalDateTime - Is global date and time (should we set all files and directories on the volume to global date and time).

DateTime - Pointer to volume global date and time structure.

OSClass - OS class to assign to the volume.
...
StarBurn_UDF2_VolumeCreate(
	...
	unsigned char OSClass/* = STARBURN_UDF_OS_CLASS_UNDEFINED*/,
	unsigned long InitialVolumeSizeInLBs/* = 0*/
	);
...
UnicodeName - Pointer to zero-terminated string with volume name (in UNICODE).
...
Volume - Pointer to volume object.

Name - Pointer to zero-terminated string with file name to store volume to.
...
SizeInUCHARs - Pointer to the variable to receive UDF volume size in unsigned chars.
...
Buffer - Pointer to data buffer to read data to.

BufferSizeInLogicalBlocks - Buffer size in logical blocks.

OffsetInLogicalBlocks - Offset within UDF volume in logical blocks.
...
Descriptor - Pointer to descriptor buffer to read data to.

DescriptorSizeInUCHARs - Descriptor buffer size in unsigned chars.
...
Directory - Pointer to directory object to destroy.
...
File - Pointer to file object to destroy.
...
Volume - Pointer to UDF volume object to destroy.
...
FileDirectory - Pointer to file or directory object.

DateTime - Pointer to input file date and time.
...
DateTime - Pointer to output file date and time.
...
Name - Directory name itself (zero-terminated string).

PathName - Directory path and name (zero-terminated string).
...
UnicodeName - Directory name itself (zero-terminated string in UNICODE format).
...
Callback - Pointer to progress callback.
...
/*
This function create UDF import volume.

Parameters

Volume - Pointer to pointer to UDF import volume object.

Root - Pointer to root directory.

GUID - Pointer to GUID.

ImportCallback - Pointer to import callback.
...
This function import UDF import volume content.

Parameters

Volume - Pointer to the UDF import volume object.
...
This function destroy created UDF import volume object.

Parameters

Volume - Pointer to UDF import volume object to destroy.
...
FileObject - Pointer to preallocated file object.

PathName - Pointer to file path and name.
...
UnicodePathName - Pointer to UNICODE file path and name.
...
IsWrite - Is write access required.
...
This fucntion closes file handle embedded to file object. 
...
FileObject - Pointer to file object.
Buffer - Pointer to data buffer to write to file.
BufferSizeInUCHARs - Buffer size in unsigned chars.
...
FileObject - Pointer to file object.
Buffer - Pointer to data buffer to read file contents to.
BufferSizeInUCHARs - Buffer size in unsigned chars.
...
FileObject - Pointer to file object.
SizeInUCHARs - Pointer to file size in unsigned chars.
...
PositionInUCHARs - Position in unsigned chars within a file.

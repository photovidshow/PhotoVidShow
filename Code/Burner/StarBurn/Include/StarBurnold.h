...
Revision/Development History:
	...
	2006-05-11 : * StarBurn_StarPort_DeviceAdd(...) renamed to StarBurn_StarPort_DeviceAddLocal(...) and now it deals
			   : with local device types only.
			   :
	...
	2006-06-20 : * Corrected StarBurn_CdvdBurnerGrabber_DiscBasicInformation_Create(..) to set right last session
               : status (was always COMPLETED) and right leadout LBA.
...
{
    INT l__INT__Length = strlen( p__PCHAR__Name );
...
{
	INT l__INT__Length = wcslen( p__PWCHAR__Name );
    ...
    l__INT__Length = ( wcslen( p__PWCHAR__Name ) - 1 );
...
Node power (file size) in UCHARs can be used to build correct tree to visualise enumeration of all the kids and walk down 
...
Please take a look at FindDevice sample to find out how StarBurn_CdvdBurnerGrabber_CreateEx could be used to create CD/DVD
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn  on the CD/DVD media and what StarBurn_CdvdBurnerGrabber_TestUnitReady can be used for. Also it's a good
...
image can be created, existing file imported and whole result burn to the CD/DVD media.
...
//
// StarBurn_UDF_GetFirstKid function is not documented
//
STARBURN_IMPEX_API
PVOID
__stdcall
StarBurn_UDF_GetFirstKid(
	IN PVOID p__PVOID__UDF__Parent
	);
...
p__PULONG__UncompressedSizeInUCHARs - Pointe to the variable to receive uncompressed payload size in UCHARs.
...
p__PVOID__ISO9660JolietFileTreeNode - Pointer to ISO9660 or Joliet file tree to get ISO9660 timestamp for.

p__PISO9660_DATE_TIME - Pointer to ISO9660 timestamp (date and time).
...
p__PULONG__SystemError - Pointer to the variable to receive system error (if function would retun EN_SYSTEM_CALL_FAILED).
...
This function reads ATIP information from the media currently inserted into CD/DVD burner device object created with the
...
p__PVOID__CdvdBurnerGrabber - Pointer to CD/DVD burner/grabber object.
...
There's no need to play with this parameter unless you really have ATAPI device in external USB enclosure and it hangs under 
heavy I/O load.
...
However if collision detection is disabled - no comparation and no actions would be performed. File would be just added to
...
trys to read beginning and end of the track to find subchannel index switch indicating EXACT track beginning or EXACT track end) 
...
drive hardware error correction cannot be disabled and reading pregap and postgap of the track is DOG SLOW) "fast read TOC"
...
This function gets DVD region mask from DVD media inserted to CD/DVD burner device object created with the call to the one of 
...
// Try to get DVD region mask from DVD media inseted to device
l__EXCEPTION_NUMBER =
...
This function gets RPC (region play code) and some additional DVD region management information from "DVD part" of the CD/DVD 
...
This example allocates CdvdBurnerGrabber object, gets DVD region management information from CD/DVD device object and destroys 
...
This function verifies recorded file system tree on CD/DVD burner device object used for this particular tree recording. This 
...
image can be burn on the CD/DVD media and how StarBurn_CdvdBurnerGrabber_VerifyTree can be used for verification of the 
...
This function verifies recorded file system image on CD/DVD burner device object used for this particular image recording (or
...
image can be burn on the CD/DVD media and how StarBurn_CdvdBurnerGrabber_VerifyFile can be used for verification of the 
recorded content. 
...
This function resets media state inside CD/DVD burner device object. This actions allow to update disc content w/o need to
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media. Adding call to StarBurn_CdvdBurnerGrabber_StopPlayScan and some of the manual media
read commands would allow faster (maybe better) recorded image verifications then built-in into the library.


Example

This example allocates CdvdBurnerGrabber object, burns sometheing and does manual verification and destroys the device object 
after it's not needed any more.
...
CD/DVD burner device in Track-At-Once mode.
...
CD/DVD media and destroys the device object after it's not needed any more.
...
Please take a look at FindDeviceEx sample to find out how StarBurn_CdvdBurnerGrabber_CreateEx could be used to create CD/DVD
burner object. Please note that StarBurn_CdvdBurnerGrabber_CreateEx would create SPTI device thus would work only under
Windows NT/2000/XP/2003. If you need Windows 95/98/Me compatibility please use StarBurn_CdvdBurnerGrabber_Create legacy call
as it's for ASPI transport (ASPI is natively supported under Windows 95/98/Me and we provide own ASPI-to-SPTI wrapper for
StarBurn).
...
Please see the SuperVideoCDEx sample that will demonstrate how SVCD/MPEG2 image can be burn on the CD/DVD media and with the 
...
This function gets last recorded track from CD/DVD media currently inseted to CD/DVD burner device object. Such a track number
...
the CD/DVD media and how StarBurn_CdvdBurnerGrabber_GetLastTrack can be used to get last recorded track number.
...
This function executes custom CDB (Command Descriptor Block) on CD/DVD burner device object. This could be used to deal with
...
Please see ExecuteGeneric sample on how to send custom SCSI commands to CD/DVD burner device.
...
// Try to send custom CDB to CD/DVD burner device
l__EXCEPTION_NUMBER =
...
This function adds the tree created from the directory to already created ISO9660 or Joliet file tree from passed directory. 
Later this tree can be used to build  "virtual" ISO96600 or Joliet file system image. Resulting "virtual" image can be either 
stored in the file on the disk or be burn directly to the CD/DVD media w/o any other intermediate operations. Unlike other
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn  on the CD/DVD media and what StarBurn_CdvdBurnerGrabber_TestUnitReadyExEx can be used for. Also it's a
...
This function gets media (inserted or not) and tray (opened or closed) statuses from CD/DVD burner device object.
...
CD/DVD media and how StarBurn_CdvdBurnerGrabber_GetMediaTrayStatus can be used to determine media and tray status.
...
This function sets read speed for CD/DVD media currently inserted to CD/DVD burner device object.
...
p__PLONG__StarPortDeviceTargetId - Pointer to the variable to receive StarPort device TargetId after device object
creation.

p__PULONG__SystemError - Pointer to the variable to receive system error if function would return anything except 
EN_SUCCESS.
...
StarBurn_StarPort_DeviceRemove, StarBurn_StarPort_DeviceListQueryLocal, StarBurn_StarPort_DeviceListQueryRemote,
...
StarBurn_StarPort_DeviceListQueryRemote API call which retreives information about REMOTE device list this call 
gets information about LOCAL device list.


Parameters

p__PSTARPORT_DEVICE_LIST - Pointer to the STARPORT_DEVICE_LIST structure drive would fill with StarPort device list 
information.

p__ULONG__StarPortDeviceListSizeInUCHARs - True STARPORT_DEVICE_LIST structure size in UCHARs.

p__PLONG__NumberOfTargets - Pointer to the variable to receive number of targets StarPort would enumerate during 
this call.
...
StarBurn_StarPort_DeviceAddLocal, StarBurn_StarPort_DeviceRemove, StarBurn_StarPort_DeviceListQueryRemote,
StarBurn_StarPort_DeviceAddRemote
...
p__PLONG__NumberOfTargets - Pointer to the variable to receive number of targets StarPort would enumerate during this 
call.
...
This function checks if the StarPortLite driver is installed and that its API version is equal to the one
StarBurn was built with.
...
This function retrieves retrieves device type, vendor ID, product ID and revision for StarPort device by its target ID.

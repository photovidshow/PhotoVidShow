...
namespace SquidgySoft.UI.Controls
{
	...
	{		
		...
		private dvdslideshowfontend.Win32.Shell32Namespaces _shell = new dvdslideshowfontend.Win32.Shell32Namespaces();		
        ...
        [System.ComponentModel.Browsable(true), System.ComponentModel.Category("Behaviour"), System.ComponentModel.Description("Display files."), System.ComponentModel.DefaultValue(false)]
		...
		public override void RequestRoot(TreeViewFolderBrowserHelper helper)
		{
			...
			switch(helper.TreeView.RootFolder)
			{
				...
				default:
					...
					TreeNodePath rootNode = CreateTreeNode(helper, root.Title,root.Self.Path,true,false, false);
		...
		{		
			...
			{
                System.Collections.ArrayList childNodeTempParent = new System.Collections.ArrayList();
				...
				{                    
                    ...
                    //						
                    TreeNodePath node = CreateTreeNode(helper, fi.Name, fi.Path, fi.IsFolder ? IsFolderWithChilds(fi) : false, ((helper.TreeView.CheckboxBehaviorMode != CheckboxBehaviorMode.None) && (parent.Checked)), false);
                    node.Tag = fi;
                    if (fi.IsFolder)
                    {
                    ...
                    {
                        childNodeTempParent.Add(node);
                    }
                }
                if (_showFiles && childNodeTempParent.Count > 0)
                {
                    foreach (TreeNode node in childNodeTempParent)
                    {
                        parent.Nodes.Add(node);
                    }
                }
		...
		protected virtual void FillMyComputer(Shell32.FolderItem folderItem, TreeNodeCollection parentCollection, TreeViewFolderBrowserHelper helper)
		{
			...
			{				
				...
				// create new node
				TreeNodePath node = CreateTreeNode(helper, fi.Name, fi.Path, IsFolderWithChilds(fi), false, false);
				node.Tag = fi;

...
#define ABR_INIT_QP ( h->param.rc.i_rc_method == X264_RC_CRF ? h->param.rc.f_rf_constant : 24 )
    ...
    rc->qp_constant[SLICE_TYPE_I] = x264_clip3( lrint(h->param.rc.i_qp_constant - rc->ip_offset), 0, 51 );
    rc->qp_constant[SLICE_TYPE_B] = x264_clip3( lrint(h->param.rc.i_qp_constant + rc->pb_offset), 0, 51 );
...
void x264_ratecontrol_start( x264_t *h, int i_force_qp, int overhead )
{
    ...
    rc->qpa_rc =
    rc->qpa_aq = 0;
    rc->qp = x264_clip3( lrintf(q), 0, 51 );
    h->fdec->f_qp_avg_rc =
    h->fdec->f_qp_avg_aq =
    rc->qpm = q;
    if( rce )
...
void x264_ratecontrol_mb( x264_t *h, int bits )
{
    ...
    if( y < h->i_threadslice_end-1 )
    {
        ...
        float qp_max, buffer_left_planned, slice_size_planned, size_of_other_slices, rc_tol;
        float step_size = 0.5;
...
int x264_ratecontrol_slice_type( x264_t *h, int frame_num )
{
    ...
    if( h->param.rc.b_stat_read )
    {
        if( frame_num >= rc->num_entries )
        {
            ...
            rc->qp_constant[SLICE_TYPE_I] = x264_clip3( lrint( qscale2qp( qp2qscale( h->param.rc.i_qp_constant ) / fabs( h->param.rc.f_ip_factor )) ), 0, 51 );
            rc->qp_constant[SLICE_TYPE_B] = x264_clip3( lrint( qscale2qp( qp2qscale( h->param.rc.i_qp_constant ) * fabs( h->param.rc.f_pb_factor )) ), 0, 51 );
...
static double clip_qscale( x264_t *h, int pict_type, double q )
{
    ...
    if( rcc->b_vbv && rcc->last_satd > 0 )
    {
        ...
        {
            double bits, qf = 1.0; double max_fill_factor, min_fill_factor;
            if( ( pict_type == SLICE_TYPE_P ||

...
static int x264_validate_parameters( x264_t *h )
{
    ...
    if( h->param.rc.i_rc_method == X264_RC_CQP )
    {
        ...
        h->param.rc.i_qp_max = x264_clip3( (int)(X264_MAX3( qp_p, qp_i, qp_b ) + .9999f), 0, 51 );
    ...
    {
        h->param.i_bframe_pyramid = X264_MIN( X264_B_PYRAMID_NONE, h->param.i_bframe_pyramid );
        ...
        h->param.analyse.i_weighted_pred = X264_MIN( h->param.analyse.i_weighted_pred, X264_WEIGHTP_BLIND );
    ...
    if( h->param.i_keyint_min == X264_KEYINT_MIN_AUTO )
        h->param.i_keyint_min = lrintf(h->param.i_keyint_max * 0.1f);
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    h->i_frame_num = 0;
    h->i_idr_pic_id = 0;
    new_timebase_den = h->param.i_timebase_den;

    if( new_timebase_den * 2 > UINT32_MAX )
    {
        x264_log( h, X264_LOG_ERROR, "Effective timebase denominator %I64u exceeds H.264 maximum\n", new_timebase_den );
        goto fail;
    }
...
static inline void x264_reference_build_list( x264_t *h, int i_poc )
{
    ...
    if( IS_X264_TYPE_B( h->fenc->i_type ) && h->param.b_bluray_compat )
        h->i_ref0 = X264_MIN( h->i_ref0, IS_X264_TYPE_B( h->fref0[0]->i_type ) + 1 );
...
static int x264_threaded_slices_write( x264_t *h )
{
    ...
    for( i = 0; i < h->param.i_threads; i++ )
    {
        ...
        height = h->mb.i_mb_height >> h->param.b_interlaced;
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    if( pic_in != NULL )
    {
		...
		// SRG Added to sort out delay dts thing
		if (fenc->i_frame == 0)

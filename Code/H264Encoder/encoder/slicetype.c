...
void x264_weights_analyse( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, int b_lookahead )
{
    ...
    fenc_var = sqrtf( fenc->i_pixel_ssd[0] );
    ref_var  = sqrtf( ref->i_pixel_ssd[0] );
    ...
    //early termination
    if( fabsf( ref_mean - fenc_mean ) < .5f && fabsf( 1.f - fenc_var / ref_var ) < epsilon )
    {
        SET_WEIGHT( weights[0], 0, 1, 0, 0 );
        return;
    }

    x264_weight_get_h264( lrintf(ref_var) ? lrintf( fenc_var/ref_var * 128.f ) : 0, 0, &weights[0] );
    found = 0;
    mindenom = weights[0].i_denom;
    minscale = weights[0].i_scale;
    minoff = 0;
    offset_search = x264_clip3( lrintf( floorf( fenc_mean - ref_mean * minscale / (1 << mindenom) + 0.5f*b_lookahead )), -128, 126 );

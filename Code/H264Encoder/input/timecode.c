...
static int parse_tcfile( FILE *tcfile_in, timecode_hnd_t *h, video_info_t *info )
{
    ...
    if( h->auto_timebase_den || h->auto_timebase_num )
    {
        ...
        h->timebase_num /= i;
        h->timebase_den /= i;
        fprintf( stderr, "timecode [info]: automatic timebase generation %I64u/%I64u\n", h->timebase_num, h->timebase_den );
    }
    ...
    if( !h->pts )
        goto fail;
    pts_seek_offset = llrint( timecodes[h->seek] * ((double)h->timebase_den / h->timebase_num) );
    h->pts[0] = 0;
    for( num = 1; num < h->stored_pts_num; num++ )
    {
        h->pts[num] = llrint( timecodes[h->seek + num] * ((double)h->timebase_den / h->timebase_num) );
...
static int open_file( char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt )
{
    ...
    if( opt->timebase )
    {
        ret = sscanf( opt->timebase, "%I64u/%I64u", &h->timebase_num, &h->timebase_den );
        if( ret == 1 )
...
static int read_frame( x264_picture_t *p_pic, hnd_t handle, int i_frame )
{
    ...
    else
    {
        ...
        p_pic->i_pts = llrint( h->last_timecode * ((double)h->timebase_den / h->timebase_num) );
    }

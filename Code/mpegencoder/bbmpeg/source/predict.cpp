...
static void predict_mb(unsigned char *oldref[], unsigned char *newref[],
      unsigned char *cur[], int lx, int bx, int by, int pict_type, int pict_struct,
      int mb_type, int motion_type, int secondfield, int PMV[2][2][2],
      int mv_field_sel[2][2], int dmvector[2]);

static void pred(unsigned char *src[], int sfield, unsigned char *dst[],
...
void predict(
unsigned char *reff[], unsigned char *refb[], unsigned char *cur[3],
int secondfield,
struct mbinfo *mbi)
{
  int i, j, k;
  ...
  if ((mb_type & MB_FORWARD) || (pict_type==P_TYPE))
  {
    ...
    if (pict_struct==FRAME_PICTURE)
    {
      ...
      else
      {
        ...
        if (verbose)
          DisplayWarning("Invalid motion_type.");
      }
    }

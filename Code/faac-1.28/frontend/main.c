...
int MainAACEncodeAndMux(int argc, char *argv[], int lengthFrames, float outputVideoFPS, int* currentAudioPercentage)
{
    int frames, currentFrame;
    faacEncHandle hEncoder;
    pcmfile_t *infile = NULL;
	double endTime = ((double)lengthFrames) / outputVideoFPS;
...
#endif
    char *faac_id_string ="aac";
    char *faac_copyright_string ="aac";
	...
	*/
    ...
    while (1) {
        ...
        c = faac_getopt_long(argc, argv, "Hhb:m:o:rnc:q:PR:B:C:I:X"
#ifdef HAVE_LIBMP4V2
                        "ws"
#endif
    ...
    case 'o':
        {
            int l = strlen(faac_optarg);
        aacFileName = malloc(l+1);
        memcpy(aacFileName, faac_optarg, l);
        aacFileName[l] = '\0';
        aacFileNameGiven = 1;
        }
        break;
        ...
        case 'I':
            sscanf(faac_optarg, "%d,%d", &chanC, &chanLF);
            break;
        case 'P':
    ...
    case ALBUM_FLAG:
        ...
        break;
    case TRACK_FLAG:
        sscanf(faac_optarg, "%d/%d", &trackno, &ntracks);
        break;
    case DISC_FLAG:
        sscanf(faac_optarg, "%d/%d", &discno, &ndiscs);
        break;
    ...
    case COMMENT_FLAG:
        ...
        break;
    case COVER_ART_FLAG: {
        FILE *artFile = fopen(faac_optarg, "rb");
...
#if 0
    case OBJTYPE_FLAG:
        if (!strcasecmp(faac_optarg, "LC"))
                objectType = LOW;
        else if (!strcasecmp(faac_optarg, "Main"))
            objectType = MAIN;
        else if (!strcasecmp(faac_optarg, "LTP")) {
            mpegVersion = MPEG4;
        objectType = LTP;
        } else
    ...
    if (!dieMessage && (argc - faac_optind) > 1 && aacFileNameGiven)
        dieMessage = "Cannot encode several input files to one output file.\n";

    if (argc - faac_optind < 1 || dieMessage)
    {
        // fprintf(stderr, dieMessage ? dieMessage : usage, progName, progName, progName, progName);
        return 1;
    }

    while (argc - faac_optind > 0) {

    /* get the input file name */
    audioFileName = argv[faac_optind++];
	...
	// ### SRG TEST
    MP4hFile = MP4Modify(aacFileName, MP4_DETAILS_ERROR, 0);

//	MP4hFile = MP4Read("Slideshow0.mp4",0xffffffff);
  ...
  //  strcpy(version_string + 5, faac_id_string);
    MP4SetMetadataTool(MP4hFile, "pvsaac1");
  //  free(version_string);
...
#endif
        if (infile->samples)
            frames = (endTime * infile->samplerate) / 1024;
        else
            frames = 0;
        currentFrame = 0;
    ...
    for (;;)
...
#endif
        {
			...
			currentTime = (((double)total_samples) / infile->samplerate);

			// ### SRG OK END EARLY I.E RACHED END TIME
            ...
            if ((showcnt <= 0) || !bytesWritten)
            {
                ...
                {
					*currentAudioPercentage = (int) ((currentTime*100/endTime)+0.4999);
...
void AACEncodeAndMux(char* waveFileName, char* videoMP4FileName, int lengthFrames, float outputVideoFPS, int* currentAudioPercentage)
{
	char* argv[9];

	argv[0] = "aacencoder";
	argv[1] = waveFileName;
    argv[2] = "-s";	 // optimize the mp4.  This help prevents pauses and breaks if the mp4 is to be streamed e.g. via html5 on Chrome.
	argv[3] = "-P";	// raw pcm
	argv[4] = "-R";	// rate
	argv[5] = "48000";
	argv[6] = "-X";	// reverse samples bytes?
	argv[7] = "-o";
	argv[8] = videoMP4FileName;

	MainAACEncodeAndMux(9, argv, lengthFrames, outputVideoFPS, currentAudioPercentage);
}

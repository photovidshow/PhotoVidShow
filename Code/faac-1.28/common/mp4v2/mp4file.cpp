...
void MP4File::Optimize(const char* orgFileName, const char* newFileName)
{
	...
	if (newFileName == NULL) {
        // ### SRG this may not work because it tries to create a temp file local to exe, which it may not have write access to.
		// m_fileName = MP4Stralloc(TempFileName());

        //
        // Create a temp file in same location as orgFileName
        // 
        m_fileName = MP4Stralloc(orgFileName);
        int length = strlen(m_fileName);
        if (length >3)
        {
            m_fileName[length-3] = 't'; // change exstention to .tmp
            m_fileName[length-2] = 'm';
            m_fileName[length-1] = 'p';
        }
    } else {

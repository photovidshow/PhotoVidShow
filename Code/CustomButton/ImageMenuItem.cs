...
{
	/// <summary>
	/// Summary description for ImageMenuItem.
	/// </summary>
	 ...
	 * ******************************************************/
		...
		/// </summary>
		static public ImageList MenuImages;

		// NiceMenu array to save the main menu item
		// so at runtime i can search an item and modify it
		private static ImageMenuItem[] myModifyNiceMenu = new ImageMenuItem[20];
		private static ImageMenuItem[] myModifyContextNiceMenu = new ImageMenuItem[20];
		...
		private Image iconblue = null;
		private static int BITMAP_SIZE = 20;
		...
		public ImageMenuItem(int image_number) : base() 
		{
			mIconIndex = image_number;
		
			OwnerDraw = true; UpdateColors();
		}
		
		private ImageMenuItem(string name) : base(name) 
		{
			OwnerDraw = true; UpdateColors();
		}
		// public for the AddRecentFile function
		public ImageMenuItem(string name, EventHandler handler) : base(name, handler) 
		{
			OwnerDraw = true; UpdateColors();
		}

		private ImageMenuItem(string name, ImageMenuItem[] items) : base(name, items) 
		{
			OwnerDraw = true; UpdateColors();
		}

		private ImageMenuItem(string name, EventHandler handler, Shortcut shortcut) : base(name, handler, shortcut) 
		{
			OwnerDraw = true; UpdateColors();
		}

		private ImageMenuItem(MenuMerge mergeType, int mergeOrder, Shortcut shortcut, string name, EventHandler onClick, EventHandler onPopup, EventHandler onSelect, ImageMenuItem[] items) : base(mergeType, mergeOrder, shortcut, name, onClick, onPopup, onSelect, items) 
		{
			OwnerDraw = true; UpdateColors();
		}

		private ImageMenuItem(string name, Image img) : this(name) 
		{
			icon = img;
		}

		private ImageMenuItem(string name, EventHandler handler, Image img) : this(name, handler) 
		{
			icon = img;
		}
		
		private ImageMenuItem(string name, EventHandler handler, Shortcut shortcut, Image img) : this(name, handler, shortcut) 
		{
			icon = img;
		}
		...
		protected override void OnMeasureItem(MeasureItemEventArgs e) 
		{
            ...
            // different depending on menu font (which is different between xp and vista)
            int max_width = (int)(e.Graphics.MeasureString("Visit PhotoVidShow Website ab", SystemInformation.MenuFont).Width);
			int extraHeight = 2;
			e.ItemHeight  = SystemInformation.MenuHeight + extraHeight;
			if ( topLevel )
				e.ItemWidth  = textwidth - 5; 
			else
                e.ItemWidth = Math.Min(max_width, textwidth + 30);
		...
		protected override void OnDrawItem(DrawItemEventArgs e) 
		{
			...
			if (this.mIconIndex!=-1 && this.icon==null && MenuImages!=null)
			{
				this.icon = MenuImages.Images[mIconIndex];
				this.iconblue = MenuImages.Images[mIconIndex+(MenuImages.Images.Count/2)];
			}
			...
			Image do_icon = this.icon;
			if (Enabled && (selected || ((e.State & DrawItemState.HotLight) > 0))) 
			{
			//	if (toplevel && selected) 
				{
				//	Console.WriteLine("Doing blue!!!");
					do_icon = this.iconblue;
				}
			}

			if (hasicon)
				DrawIcon(g, do_icon, bounds, selected, Enabled, Checked);
			else
			{
				if (Checked)
		...
		{
			int iconTop = bounds.Top + 1+ (itemHeight - BITMAP_SIZE)/2;
		...
		private void BuildMenuTree(IList myMenu, MenuItem parentMenu, NiceMenuClickEvent yourClickFunction)
		{
			foreach (MenuItem myMenuItem in myMenu) 
			{
				...
				if (AddMenuImage == true)
					newSubMenu = new ImageMenuItem(myMenuItem.Text, new EventHandler(yourClickFunction), myMenuItem.Shortcut, MenuImages.Images[Convert.ToInt32(IndexImage)]);
				else
					newSubMenu = new ImageMenuItem(myMenuItem.Text, new EventHandler(yourClickFunction), myMenuItem.Shortcut);	
		...
		private void BuildMenuTree(IList myMenu, MainMenu parentMenu, NiceMenuClickEvent yourClickFunction)
		{
			...
			foreach (MenuItem myMenuItem in myMenu) 
			{
				ImageMenuItem newMainMenu = new ImageMenuItem(myMenuItem.Text);
		...
		private void BuildMenuTree(IList myMenu, ContextMenu parentMenu, NiceMenuClickEvent yourClickFunction)
		{
			...
			foreach (MenuItem myMenuItem in myMenu) 
			{
				...
				if (AddMenuImage == true)
					newMainMenu = new ImageMenuItem(myMenuItem.Text, new EventHandler(yourClickFunction), myMenuItem.Shortcut, MenuImages.Images[Convert.ToInt32(IndexImage)]);
				else
					newMainMenu = new ImageMenuItem(myMenuItem.Text, new EventHandler(yourClickFunction), myMenuItem.Shortcut);	
		 ...
		 *  at runtime
		 * ****************************************************/
		public ImageMenuItem SearchNiceMenuItem(string NiceMenuText)
		{
			contModify = 0;
		repeat:
			IList myMenuList = new Menu.MenuItemCollection(myModifyNiceMenu[contModify]);
			foreach (ImageMenuItem myMenuItem in myMenuList) 
			{
				// ---------------------------------------
				...
				if (myMenuItem.IsParent == true)
				{
					...
					foreach (ImageMenuItem myMenuItem2 in myMenuList2) 
					{
						// -------------------------------- 
		 ...
		 * ****************************************************/
		public ImageMenuItem SearchContextNiceMenuItem(string ContextNiceMenuText)
		{
			contModifyContext = 0;
			repeat:
			// ---------------------------------------
